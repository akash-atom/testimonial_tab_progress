...
function setupFinsweetCompatibility() {
    ...
    const watchForFinsweetTabs = ()=>{
        ...
        if (finsweetTabs.length > 0 || customTabLinks.length > 0) {
            ...
            tabPanes.forEach((pane)=>{
                if (pane.children.length > 0 && pane.textContent.trim() !== '') hasContent = true;
            });
            if (hasContent) {
                initProgressBarAnimation();
                isInitialized = true;
    ...
    const tabsContainer = document.querySelector('[fs-list-element="tabs"]') || document.querySelector('[data-fs-list-element="tabs"]');
    if (tabsContainer) {
        const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (mutation.type === 'childList') // Check for both Finsweet-generated tabs and our custom elements
                {
                    if (watchForFinsweetTabs()) observer.disconnect();
                }
            });
        });
            ...
            mutations.forEach(function(mutation) {
                if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                    ...
                    if (target.classList.contains('w-tab') || target.classList.contains('w--current')) {
                        if (watchForFinsweetTabs()) webflowObserver.disconnect();
                    }
                }
            });
        });
    ...
    const aggressiveCheck = setInterval(()=>{
        attempts++;
        if (watchForFinsweetTabs()) clearInterval(aggressiveCheck);
        else if (attempts >= maxAttempts) clearInterval(aggressiveCheck);
    }, 500);
    ...
    const contentObserver = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                ...
                tabPanes.forEach((pane)=>{
                    if (pane.children.length > 0 && pane.textContent.trim() !== '') hasNewContent = true;
                });
                if (hasNewContent) setTimeout(()=>{
                    watchForFinsweetTabs();
                }, 200);
            }
        });
    ...
    setTimeout(()=>{
        ...
        if (firstTab && !firstTab.classList.contains('w--current')) firstTab.click();
...
function initProgressBarAnimation() {
    ...
    if (tabLinks.length === 0) {
        ...
        if (tabLinks.length === 0) // Try other possible selectors
    ...
    const observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            if (mutation.type === 'attributes' && mutation.attributeName === 'class') updateProgressBars();
        });
    });
...
function updateProgressBars() {
    ...
    let tabLinks = document.querySelectorAll('.test_tab_link');
    if (tabLinks.length === 0) tabLinks = document.querySelectorAll('[fs-list-element="tabs"] .w-tab');
    if (tabLinks.length === 0) tabLinks = document.querySelectorAll('.w-tab');
    tabLinks.forEach((tabLink, index)=>{
        // Try multiple ways to find the progress bar
        let progressBar = tabLink.querySelector('#progress_bar');
        if (!progressBar) // Try looking for progress bar by class or other attributes
        ...
        if (isActive) {
            ...
            } else // Start new animation from 0% to 100% over 5 seconds
            gsap.to(progressBar, {
                width: '100%',
                duration: 5,
                ease: 'power2.out'
...
function setupHoverPause() {
    ...
    // If no custom class found, try Finsweet's generated structure
    if (tabPanes.length === 0) tabPanes = document.querySelectorAll('[fs-list-element="tabs"] .w-tab-pane');
    if (tabPanes.length === 0) tabPanes = document.querySelectorAll('.w-tab-pane');
...
function pauseProgressBars() {
    ...
    if (tabLinks.length === 0) tabLinks = document.querySelectorAll('.w-tab');
    tabLinks.forEach((tabLink, index)=>{
        const progressBar = tabLink.querySelector('#progress_bar') || tabLink.querySelector('.progress_bar') || tabLink.querySelector('[data-progress-bar]') || tabLink.querySelector('.w-tab-content #progress_bar');
...
function startAutoSwitchTimer() {
    ...
    if (!isPaused) // Set timer for 5 seconds (same as progress bar duration)
...
function switchToNextTab() {
    ...
    const currentTabIndex = Array.from(tabLinks).findIndex((tab)=>tab.classList.contains('w--current'));
